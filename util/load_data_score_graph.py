...
class Loader(object):
    def __init__(self, flag):
        if platform.system() == 'Linux':
            self.path_init_file = path_init_file = '/home/cuizeyu/pythonfile/CTR_GNN/data/azenuz/'
            self.path_file = path_file = './data/azenuz/'
            # path_init_file = '/home/hufenyu/Documents/pythonfile/dataset/'
            # path_file = '/home/hufenyu/Documents/pythonfile/lightrnn_czy/data/'

        else:
            self.path_init_file = path_init_file = '/Users/czy_yente/PycharmProjects/CTR_GNN/data/azenuz/'
            self.path_file = path_file = './data/azenuz/'

        self.data_split_num = 10
    ...
    def change_data_list(self, idx):
        ...
        temp_zip_1 = filter(lambda x: x[1] == '1', temp_zip)
        temp_zip_0 = filter(lambda x: x[1] == '0', temp_zip)
    ...
    def load_feature_id(self, path_):
        ...
        att = attribte_list[:]
        att = filter(lambda x: x != u'id' and x != u'click', att) # 将 x != u'id' and x != u'click'的特征保存
        each_feature_id = [feature_id[at] for at in att]  # 拿到的每列数据拼在一起[["site_id":1fbe01fe, f3845767, f028772b....]
                                                                                #["C20":1000009418151094273,  10000918755742328737, 100a0d9187557423287aq2, ...]]
        feature_num = [len(temp) for temp in each_feature_id] # config.feature_num= [3000,3000,3000....3000] 22个特征
